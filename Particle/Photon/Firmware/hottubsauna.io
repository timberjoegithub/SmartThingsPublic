...
// New_York - Boston time zone GMT-3
...
#define TEMPERATURE_SAMPLE_INTERVAL 60 * MILLISECONDS_TO_SECONDS
elapsedMillis temperatureSampleInterval;

//temperature related variables
#define INVALID -1
double temperatureCurrent = INVALID;
double temperatureTarget = 95;
...
double temperatureTarget2 = 128.0;
...
NCD8Relay relayController;
  int HotTubSwitch(String command);
  int SaunaSwitch(String command);
  int HotTubLightSwitch(String command);
  int LightSwitch(String command);
  int Relay5Switch(String command);
  int Relay6Switch(String command);
  int Relay7Switch(String command);
  int Relay8Switch(String command);
int r1 = D0; 
int r2 = D1;
int r3 = D2;
int r4 = D3;
...
int r7 = D6;
int r8 = D7;
  //Set hot tub and sauna FSM to not start by default
  bool overrideFSM1 = true;
  bool overrideFSM2 = true;
...
void setup()
{
  ...
  *******************************************************************************/
  ...
  //Added functions to control other relays (assuming 8, add or subtract for your config)
  Particle.function("HotTubSwitch", hotTubSwitch);
  Particle.function("SaunaSwitch", saunaSwitch);
  Particle.function("HTLghtSwitch", hotTubLightSwitch);
  Particle.function("LightSwitch", lightSwitch);
  Particle.function("Relay5Switch", relay5Switch);
  Particle.function("Relay6Switch", relay6Switch);
  Particle.function("Relay7Switch", relay7Switch);
  Particle.function("Relay8Switch", relay8Switch);
...
void quickLoop()
{
  ...
  if (overrideFSM1 == false) {stateMachine1.update();};
  if (overrideFSM2 == false) {stateMachine2.update();};
}
...
int setTarget(String parameter)
{
  ...
  if ((localValue >= -30) && (localValue <= 160))
  {
...
int setTarget2(String parameter)
{
  ...
  if ((localValue >= -30) && (localValue <= 108))
  {
...
int relay5Switch(String command)
{
  if (command == "1") 
	...
	relayController.turnOffRelay(5);
	Serial.println("returning");
    return 0;
    }
}
...
int relay7Switch(String command)
{
  if (command == "1") 
	...
	relayController.turnOffRelay(7);
	Serial.println("returning");
    return 0;
    }
}
int relay8Switch(String command)
{
  if (command == "1") 
	...
	relayController.turnOffRelay(8);
	Serial.println("returning");
    return 0;
    }
}
...
{
  // is time up? no, then come back later
  if ((stateMachine1.timeInCurrentState()) < (unsigned(initTimeout * MILLISECONDS_TO_SECONDS)))
  {
    return;
  }
...
void offUpdateFunction()
{
  ...
  if ((stateMachine1.timeInCurrentState()) < (unsigned(minimumTimeInState * MILLISECONDS_TO_MINUTES)))
  {
    return;
  }
...
{
  // is time up? no, then come back later
  if ((stateMachine2.timeInCurrentState()) < (unsigned(initTimeout * MILLISECONDS_TO_SECONDS)))
  {
    return;
  }
...
void offUpdateFunction2()
{
  ...
  // is minimum time up? no, then come back later
  if ((stateMachine2.timeInCurrentState()) < (unsigned(minimumTimeInState * MILLISECONDS_TO_MINUTES)))
  {
    return;
  }
